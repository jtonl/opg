...
echo "$VALID_REQUEST" | opa eval -d policies/api_security.rego "data.api.security.decision" -I --format pretty
...
echo "$INVALID_REQUEST" | opa eval -d policies/api_security.rego "data.api.security.decision" -I --format pretty
...
echo "$PROD_DEPLOYMENT" | opa eval -d policies/deployment.rego "data.deployment.security.decision" -I --format pretty
...
echo "$INVALID_PROD_DEPLOYMENT" | opa eval -d policies/deployment.rego "data.deployment.security.decision" -I --format pretty
